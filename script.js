...
function applyImportedConfig(settingsObj) {
    for (const key in settingsObj) {
        if (key.startsWith('sp_')) {
            localStorage.setItem(key, settingsObj[key]);
        }
    }
...
// Run this early so if a user hits a magic link, it imports and reloads immediately
async function checkUrlImport() {
    ...
    if (importId) {
        ...
        if (hasExistingSetup) {
            // Setup exists, confirm before overwriting
            ...
            document.getElementById('btnCancelMagicLink').onclick = () => { 
                modal.classList.remove('show'); 
                // Remove the ?import= param from the URL cleanly
                window.history.replaceState({}, document.title, window.location.pathname); 
            };
        } else {
            // New user, import automatically
            performImport();
        }
    }
}
...
function setActiveProvider(key) {
    ...
    searchForm.action = provider.action; 
    searchForm.method = provider.method; 
    searchInput.name = provider.inputName; 
    activeIcon.src = provider.icon; 
    activeIcon.alt = provider.name;
...
dropdown.querySelectorAll('li').forEach(item => { 
    item.addEventListener('click', (e) => { 
        setActiveProvider(e.currentTarget.getAttribute('data-provider')); 
        searchInput.focus(); 
    }); 
});
...
document.addEventListener('keydown', (e) => {
    if (e.altKey && e.key.toLowerCase() === 's') {
        ...
        // Briefly show the dropdown to provide visual feedback
...
async function fetchSuggestions(query, rawInput) {
    ...
    if (rule === 'match') {
        ...
        if (action.includes('google')) providerToUse = 'google'; 
        else if (action.includes('duckduckgo')) providerToUse = 'duckduckgo'; 
        else if (action.includes('startpage')) providerToUse = 'startpage'; 
    ...
    try {
        ...
        if (data && data.length > 1 && data[1].length > 0) {
            renderSuggestions(data[1].slice(0, 8), rawInput); 
        } else {
            closeSuggestions();
        }
    } catch (e) { 
...
function renderSuggestions(suggestions, rawInput) {
    ...
    if (suggestions.length > 0 && rawInput) {
        ...
        if (validGhost) {
            autocompleteOverlay.value = rawInput + validGhost.substring(rawInput.length);
        }
    }
...
searchInput.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Tab' && autocompleteOverlay.value && autocompleteOverlay.value !== searchInput.value) { 
        e.preventDefault(); 
        searchInput.value = autocompleteOverlay.value; 
        const raw = searchInput.value; 
        fetchSuggestions(raw.trim(), raw); 
        return; 
    }
...
function updateSuggestionSelection(items) {
    ...
    if (currentSelectedIndex >= 0) { 
        ...
        searchInput.value = selected.querySelector('span').innerText; 
        autocompleteOverlay.value = ''; 
    }
}
...
searchInput.addEventListener('input', (e) => {
    ...
    if (autocompleteOverlay.value && !autocompleteOverlay.value.toLowerCase().startsWith(rawVal.toLowerCase())) {
        autocompleteOverlay.value = '';
    }
    
    const trimVal = rawVal.trim(); 
    if (!trimVal || trimVal.startsWith('!')) { 
        closeSuggestions(); 
        return; 
    }
...
function showConsoleError(errorMessage, originalQuery) { 
    ...
    originalSearchText = originalQuery; 
    searchInput.value = errorMessage; 
    isErrorState = true; 
    searchInput.blur(); 
}
...
function restoreSearch() { 
    if (isErrorState) { 
        ...
        searchInput.value = originalSearchText; 
        isErrorState = false; 
    } 
}

searchInput.addEventListener('focus', restoreSearch); 
searchInput.addEventListener('click', restoreSearch);
...
async function checkUrlAndRedirect(url, originalQuery) {
    try {
        ...
        if (!response.ok) { 
            showConsoleError(`[Console] Error ${response.status}: ${response.statusText || 'Not Found'}`, originalQuery); 
            return; 
        }
        window.location.href = url;
    } catch (error) {
        if (error.name === 'AbortError') { 
            showConsoleError(`[Console] Error 522: Connection Timed Out`, originalQuery); 
            return; 
        }
        try { 
            // Fallback for CORS blocks on HEAD requests
            await fetch(url, { method: 'HEAD', mode: 'no-cors' }); 
            window.location.href = url; 
        } catch (fallbackError) { 
            showConsoleError(`[Console] Error: Host Unreachable or Connection Refused`, originalQuery); 
        }
    }
}
...
searchForm.addEventListener('submit', (e) => {
    ...
    if (matchedAlias) {
        ...
        if (targetUrl.includes('%s')) {
            targetUrl = targetUrl.replace('%s', encodeURIComponent(remainingQuery)); 
        }
    ...
    if (hasProtocol || isLocal || isDomain) { 
        e.preventDefault(); 
        searchInput.classList.add('loading'); 
        checkUrlAndRedirect(hasProtocol ? query : 'https://' + query, query); 
    } else { 
        // 3. Fallback standard web search
        searchInput.classList.add('loading'); 
    }
});
...
function renderBookmarks() {
    ...
    for (let i = 1; i <= 8; i++) {
        const url = localStorage.getItem(`sp_bm${i}_url`) || ''; 
        const icon = localStorage.getItem(`sp_bm${i}_icon`) || '';
...
async function updateBackground(rawUrl, forceWarp = null) {
    ...
    if (enableWarp) {
        ...
        try { 
            ...
            console.error("Kawarp failed:", error); 
        }
    } else {
        if (kawarpInstance) { 
            kawarpInstance.stop(); 
...
async function loadSettings() {
    ...
    bg1.style.filter = `blur(${savedBlur}px)`; 
    bg2.style.filter = `blur(${savedBlur}px)`; 
    ...
    // Setup Idle configuration variables
    idleTimeoutConfig = parseInt(localStorage.getItem('sp_idle_timeout') || '5') * 1000;
    
    if (localStorage.getItem('sp_idle_hide_mouse') !== 'false') {
        document.body.classList.add('idle-hide-mouse');
    }
    if (localStorage.getItem('sp_idle_hide_logo') !== 'false') {
        document.body.classList.add('idle-hide-logo');
    }
    if (localStorage.getItem('sp_idle_hide_search') !== 'false') {
        document.body.classList.add('idle-hide-search');
    }
...
document.getElementById('obOpenImportBtn').addEventListener('click', (e) => { 
    e.preventDefault(); 
    // Hide main onboarding wizard
    document.getElementById('onboardingOverlay').classList.remove('show');
    // Show import menu
    document.getElementById('obImportOverlay').classList.add('show'); 
});

document.getElementById('obCloseImportBtn').addEventListener('click', () => { 
    // Hide import menu
    document.getElementById('obImportOverlay').classList.remove('show'); 
    // Re-show main onboarding wizard
...
document.getElementById('obSyncCloudBtn').addEventListener('click', async (e) => {
    const id = document.getElementById('obSyncShortId').value.trim(); 
    ...
    try {
        ...
        alert("Import failed. ID may be invalid or expired."); 
    } finally { 
        btn.innerText = orig; 
    }
});
...
document.getElementById('obSyncB64Btn').addEventListener('click', () => {
    const hash = document.getElementById('obSyncB64').value.trim(); 
    ...
    try { 
        ...
        alert("Invalid Base64 hash payload."); 
    }
});

document.getElementById('obSyncJsonBtn').addEventListener('click', () => {
    const rawJson = document.getElementById('obSyncJson').value.trim(); 
    if (!rawJson) return alert("Please paste valid JSON.");
    
    try { 
        applyImportedConfig(JSON.parse(rawJson)); 
    } catch (err) { 
        alert("Invalid JSON payload. Check syntax."); 
    }
});
...
async function initOnboarding() {
    ...
    document.getElementById('obBtnNo').onclick = () => { 
        ...
        const match = Array.from(browserSelect.options).find(opt => opt.value === chosenLogo); 
...
// --- Focus & Idle Logic ---
function forceFocus() {
    // Do not steal focus if an onboarding or import modal is currently open
    if (document.querySelector('.onboarding-overlay.show')) return;
    ...
    if (searchInput) { 
        ...
        setTimeout(() => {
            // Double check again just in case a modal opened during the delay
            if (!document.querySelector('.onboarding-overlay.show')) {
                searchInput.focus();
            }
        }, 50); 
    }
...
document.addEventListener('keydown', (e) => {
    ...
    // Check if the user is already safely typing inside another input field
    const activeTag = e.target.tagName;
    const isInputActive = activeTag === 'INPUT' || activeTag === 'TEXTAREA' || activeTag === 'SELECT';

    // Only snap focus to the main search bar if they aren't currently using a different text box
    if (isTypingChar && !isInputActive && document.activeElement !== searchInput) {
        // Also ensure no modals are open before snapping focus
        if (!document.querySelector('.onboarding-overlay.show')) {
            searchInput.focus();
        }
    }
